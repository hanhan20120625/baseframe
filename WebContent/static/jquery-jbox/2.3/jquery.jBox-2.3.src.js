...
(function ($) {
    $.jBox = function (content, options) {
        ...
        $.each(content, function (stateName, stateOptions) {
            ...
            states.push('<div style="min-width:50px;width:' + (typeof options.width == 'number' ? options.width + 'px' : 'auto') + '; height:' + contentHeight + ';">' + loadingHtml + '<div id="jbox-content" class="jbox-content" style="width:' + (typeof options.width == 'number' ? options.width + 'px' : 'auto') +'; height:' + contentHeight + ';overflow:hidden;overflow-y:auto;">' + stateOptions.content + '</div></div>');
